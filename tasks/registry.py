...
TASK_REGISTRY: dict[str, TaskSpec] = {
    ...
    "system": TaskSpec(
        ...
        description=(
            ...
            "open_path (target=file or folder — files only, never URLs; "
            "open a URL with open_application or open_app, which hand it "
            "to the browser), brightness (operation=get|set|up|down, "
            "level=1..100, "
    ...
    "agent_task": TaskSpec(
        ...
        description=(
            ...
            "or in-depth research and comparison. "
            "Work happens in its own folder — projects/<label> under the "
            "workspace, unless the user names a directory. The assistant "
            "REMEMBERS its projects across restarts, so a project can be "
            "picked back up by name and the agent keeps its whole context. "
            "Actions: 'delegate' "
            "(task=the user's full objective, in their own words; "
            "directory=where to build it if the user named a place, e.g. "
            "'~/projects/jarvis_frontend'; optional label) "
            "starts a BRAND-NEW piece of work — use it only when the "
            "user asks for a new project or a task that has never been "
            "started; 'follow_up' "
            "(instruction=what to change or add; optional label) "
            "continues work already in progress and is what you use when "
            "the user refines, changes or extends something the agent is "
            "building — no label continues the current project, a label "
            "resumes any known project by name, and a small change must "
            "NEVER open a new session and lose the context; 'status' "
