...
RESPONSE_LLM_INSTRUCTIONS = """
CRITICAL SYSTEM INSTRUCTION: If the user's request requires an external tool, hardware action, or system command (such as opening apps, setting/listing/canceling alarms, checking the weather, putting the computer/PC to sleep, restarting, shutting down, creating/reading/writing/deleting files, moving windows, wallpaper changes, system monitoring commands, or web searches), you must NOT attempt to fulfill it yourself or state that you cannot do so. Instead, generate a short, immediate acknowledgment in the present continuous tense indicating that you are starting that action right now.
Examples of acknowledgments for tool queries:
- "Opening WhatsApp..." (or the specific app name)
- "Searching for that..." (or similar search acknowledgment)
- "Working on it..."
- "Checking that now..."
- "Starting the process..."
Never generate completion messages such as "Done.", "Completed.", "Finished.", "Message sent.", or "Deleted." in your response. The response layer should not know whether the execution succeeds or fails.
"""
...
class ReasoningCoordinator:
    ...
    async def on_user_input(self, event: TextInputReceived | TranscriptReady) -> None:
        """Handle incoming user input immediately by running the Response LLM pipeline."""
        ...
        try:
            ...
            retrieved = await self._retrieve_rag_context(text, intent_event=None)

            # 4. Build system prompt
            system = self._build_system_prompt(memory_event, retrieved)

            # Append parallel response instructions to the system prompt
            system = f"{system}\n\n{RESPONSE_LLM_INSTRUCTIONS}"

            # 5. Stream LLM conversational reply
            history = self._get_history()

            logger.debug(
                "Calling Response LLM (streaming) | history_turns=%d | system_len=%d | rag_chunks=%d",
                len(history),
                len(system),
                len(retrieved),
            )
            ...
            try:
                ...
                stream_failed = True
                logger.warning(
                    "Streaming Response LLM call failed (session=%s): %s", session_id, exc
                )
            ...
            if not response_text:
                if stream_failed or raw_parts:
                    logger.info(
                        "Empty LLM stream (session=%s); retrying once",
                        session_id,
                    )
                    try:
                        raw_parts.clear()
            ...
            if not response_text:
                response_text = "Sorry, sir — my response came back blank. Could you say that again?"

            # Add assistant turn to short-term memory
            self.memory_manager.short_term.add("assistant", response_text)

            # Persist response memory
            await self._persist_response_memory(payload, session_id=session_id)

            # Mark stream done + emit final response
