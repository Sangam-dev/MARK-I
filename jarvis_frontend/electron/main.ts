...
function animateBounds(win: BrowserWindow, from: Electron.Rectangle, to: Electron.Rectangle, ms: number, onDone?: () => void) {
  ...
  animTimer = setInterval(() => {
    ...
    if (step >= steps) { clearInterval(animTimer!); animTimer = null; win.setBounds(to); onDone?.() }
  }, TICK)
}
...
function enterCompact() {
  ...
  isCompact = true; isAnimating = true
...
function exitCompact() {
  ...
  isCompact = false; isAnimating = true
...
ipcMain.on('manual-compact', () => { if (!isCompact && !isAnimating) enterCompact() })
ipcMain.on('restore-full',   () => { if (isCompact && !isAnimating) exitCompact() })  // cube click → restore
